...
CSCCathodeLCTProcessor::CSCCathodeLCTProcessor(unsigned endcap,
  ...
  if (isSLHC) {
    hit_persist_my  = conf.getUntrackedParameter<unsigned int>("clctHitPersistMy",6);
    nplanes_hit_pretrig_my =
      conf.getUntrackedParameter<unsigned int>("clctNplanesHitPretrigMy",2);
    pid_thresh_pretrig_my =
      conf.getUntrackedParameter<unsigned int>("clctPidThreshPretrigMy",2);
    min_separation_my    =
      conf.getUntrackedParameter<unsigned int>("clctMinSeparationMy",8);

    start_bx_shift = conf.getUntrackedParameter<int>("clctStartBxShift",0);
    
    // use of localized dead-time zones
    use_dead_time_zoning = conf.getUntrackedParameter<bool>("useDeadTimeZoning",true);
  ...
  early_tbins_my = ctmb.getUntrackedParameter<unsigned int>("tmbEarlyTbins",4);
...
CSCCathodeLCTProcessor::run(const CSCComparatorDigiCollection* compdc) {
  ...
  if (numStrips == 0) {
	...
	if ( (smartME1aME1b || naiveME1aME1b) && !disableME1a && theRing == 1 ) numStrips = 64;
	if ( (smartME1aME1b || naiveME1aME1b) && !disableME1a && theRing == 4 ) numStrips = 48;
      }
...
void CSCCathodeLCTProcessor::run(
  ...
  if (isTMB07) { // TMB07 (latest) version of the CLCT algorithm.
    if (!use_dead_time_zoning) LCTlist = findLCTs(halfstrip);
    else LCTlist = findLCTsSLHC(halfstrip);
  }
  ...
  for (int i_layer = 0; i_layer < CSCConstants::NUM_LAYERS; i_layer++) {
	...
	if (bx_times[i] > 1 && bx_times[i] < static_cast<int>(fifo_tbins)) {
	  ...
	  if (i == 0 || (i > 0 && bx_times[i]-bx_times[i-1] >=
			 static_cast<int>(hit_persist_my))) {
...
bool CSCCathodeLCTProcessor::preTrigger(const std::vector<int> strip[CSCConstants::NUM_LAYERS][CSCConstants::NUM_HALF_STRIPS],
					...
					int& first_bx)
{
  static const int hs_thresh = nplanes_hit_pretrig_my;
  static const int ds_thresh = nplanes_hit_pretrig_my;
...
bool CSCCathodeLCTProcessor::hitIsGood(int hitTime, int BX) {
  ...
  if (dt >= 0 && dt <= static_cast<int>(hit_persist_my)) {return true;}
  else {return false;}
...
void CSCCathodeLCTProcessor::priorityEncode(
  ...
  static const unsigned int hs_thresh = nplanes_hit_pretrig_my;
 ...
 unsigned int pulse[CSCConstants::NUM_LAYERS][CSCConstants::NUM_HALF_STRIPS]) {
  ...
  for (int i_layer = 0; i_layer < CSCConstants::NUM_LAYERS; i_layer++) {
	...
	for (unsigned int i = 0; i < bx_times.size(); i++) {
	  ...
	  if (bx_times[i] < 0 || bx_times[i] + hit_persist_my >= bits_in_pulse) {
	  ...
	  if (bx_times[i] >= start_bx_shift)
	    for (unsigned int bx = bx_times[i];
	         bx < bx_times[i] + hit_persist_my; bx++)
...
bool CSCCathodeLCTProcessor::preTrigger(
  ...
  for (unsigned int bx_time = start_bx; bx_time < fifo_tbins; bx_time++) {
	...
	if (nhits[hstrip]    >= nplanes_hit_pretrig_my &&
	    best_pid[hstrip] >= pid_thresh_pretrig_my) {
	  pre_trig = true;
	}
      }
...
bool CSCCathodeLCTProcessor::ptnFinding(
  ...
  if (layers_hit < nplanes_hit_pretrig_my) return false;
...
void CSCCathodeLCTProcessor::markBusyKeys(const int best_hstrip,
					  const int best_patid,
                                int quality[CSCConstants::NUM_HALF_STRIPS]) {
  int nspan = min_separation_my;
  int pspan = min_separation_my;
...
std::vector<CSCCLCTDigi> CSCCathodeLCTProcessor::findLCTsSLHC(const std::vector<int> halfstrip[CSCConstants::NUM_LAYERS][CSCConstants::NUM_HALF_STRIPS]) {
  ...
  for (int i=0; i<CSCConstants::NUM_HALF_STRIPS; i++)
    for (int j=0; j<MAX_CLCT_BINS; j++) busyMap[i][j] = false;
  ...
  while (start_bx < stop_bx) {
	...
	for (int hstrip = stagger[CSCConstants::KEY_CLCT_LAYER-1];
	  ...
	  // bends have equal quality).
	  quality[hstrip] = (best_pid[hstrip] & 14) | (nhits[hstrip] << 5);
	  if (quality[hstrip] > best_quality[0] && !busyMap[hstrip][first_bx]) {
	    best_halfstrip[0] = hstrip;
	    best_quality[0]   = quality[hstrip];
	    if (infoV > 1) {
	      LogTrace("CSCCathodeLCTProcessor")
	...
	markBusyKeys(best_halfstrip[0], best_pid[best_halfstrip[0]], quality);
        ...
        for (int hstrip = stagger[CSCConstants::KEY_CLCT_LAYER-1];
	   hstrip < maxHalfStrips; hstrip++) {
	  if (quality[hstrip] > best_quality[1] && !busyMap[hstrip][first_bx]) {
	    best_halfstrip[1] = hstrip;
	    best_quality[1]   = quality[hstrip];
	    if (infoV > 1) {
	      LogTrace("CSCCathodeLCTProcessor")
	...
	if (ptn_trig) {
	  // Once there was a trigger, CLCT pre-trigger state machine
	  // checks the number of hits that lie on a key halfstrip pattern template
	  // at every bx, and waits for it to drop below threshold.
          // During that time no CLCTs could be found with its key 
          // halfstrip in the area of
          // [clct_key-clct_state_machine_zone, clct_key+clct_state_machine_zone]
	  // The search for CLCTs resumes only when the number of hits
	  // drops below threshold.
          for (unsigned int ilct=0; ilct<lctListBX.size(); ilct++){
            int key_hstrip = lctListBX[ilct].getKeyStrip();
            int min_hstrip = key_hstrip - clct_state_machine_zone;
            if (min_hstrip < stagger[CSCConstants::KEY_CLCT_LAYER-1])
              min_hstrip = stagger[CSCConstants::KEY_CLCT_LAYER-1];
            int max_hstrip = key_hstrip + clct_state_machine_zone;
	    ...
	    for (int bx = latch_bx + 1; bx < stop_time; bx++) {
              bool busy_bx = false;
              if (bx <= latch_bx) busy_bx = true; // always busy before drift time
              if (!busy_bx){
                bool hits_in_time = ptnFinding(pulse, maxHalfStrips, bx);
